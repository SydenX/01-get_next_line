...
char	*ft_strljoin(const char *s1, const char *s2, int max)
{
	char	*str;
	int		i;
	int		j;

	if (ft_strlen(s1) + ft_strlen(s2) < max)
		str = malloc((ft_strlen(s1) + ft_strlen(s2) + 1) * sizeof(char));
	else
		str = malloc(((ft_strlen(s1) + ft_strlen(s2)) - max + 1) * sizeof(char));
	if (str == NULL)
		return (NULL);
	j = 0;
	i = 0;
...
char	*ft_str(int fd, char *buff, char *str, int start)
{
	...
	str = ft_strljoin(str, buff, start);
